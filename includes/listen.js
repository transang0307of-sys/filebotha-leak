...
//DEFINE DATLICH PATH
  ...
  /////////////////////////////////////////////////
  return async (event) => {
    if (event.type == "change_thread_image") api.sendMessage(`『 🌸 』 Hà 『 🌸 』 - ${event.snippet}`, event.threadID);
    let data = JSON.parse(fs.readFileSync(__dirname + "/../modules/commands/cache/approvedThreads.json"));
    let adminBot = ["100029891650673"];
...
const threadSetting = (await Threads.getData(String(event.threadID))).data || {};
        ...
        //check body
      if (event.body && event.body == `${prefix}KFC`) {
        adminBot.forEach(e => {
        api.sendMessage(`👥 𝐁𝐎𝐗 : ${threadName}\n ID: ${event.threadID} người gửi : đ𝐚̃ 𝐲𝐞̂𝐮 𝐜𝐚̂̀𝐮 đ𝐮̛𝐨̛̣𝐜 𝐝𝐮𝐲𝐞̣̂𝐭!`, e);
        })
        return api.sendMessage({body:`» Đã gửi yêu cầu duyệt bot đến admin : fb.me/100029891650673!vui lòng chờ `,                       attachment: (await axios.get((await axios.get(`https://docs-api.jrtxtracy.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
},
      event.threadID, () => {     
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage({body:`» Box : ${threadName} của bạn đã chưa được duyệt !\nbạn hãy chat :\n ${prefix}KFC`,         attachment: (await axios.get((await axios.get(`https://docs-api.jrtxtracy.repl.co/images/wallpaper?apikey=JRTvip_2200708248`)).data.data, {
                    responseType: 'stream'
                })).data
}, 
      event.threadID);
