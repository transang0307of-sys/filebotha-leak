...
const { threadID, messageID, senderID } = event;
  if (!args[0]) return api.sendMessage({body:`== [ GÓP Ý ] ==\n━━━━━━━━━━━━\n→ CÁCH DÙNG\n→ gopy send (gửi nd)\n→gopy find (tìm người góp ý)\n→gopy list (danh sách góp ý) ` }, event.threadID,event.messageID);
	switch (args[0]) {
		case 'send': {
			return api.sendMessage(`→ Reply tin nhắn này để nhập nội dung bạn muốn gửi lên CFS!`, threadID, (error, info) => {
		...
		case 'find': {
			...
			var res = (await axios.get(`https://nguyenlienmanh.com/cfsdata`)).data
				res.reverse()
...
const { threadID, messageID, senderID, body } = event;
	switch (handleReply.type) {
		case 'send': {
			...
			var res = await axios.get(`https://nguyenlienmanh.com/cfs?message=${encodeURI(body)}&senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`);
			return api.sendMessage('→ Gửi nội dung này lên CFS thành công', threadID, messageID)
		}
		case 'list': {
...
const { threadID, messageID, senderID } = event;
	switch (handleReaction.type) {
		case 'react': {
			var res = await axios.get('https://nguyenlienmanh.com/react?index=' + handleReaction.index);
		}
	}
}