  ...
  try {
    ...
    case "i":{
      ...
      if (!username) return api.sendMessage("con mẹ m info đâu", threadID);
       const res = await axios.get(`https://api.nguyenlienmanh.com/instagram/info?username=${username}`);
...
const res = await axios.get(`https://api.nguyenlienmanh.com/instagram/videodl?url=${link}`);
var url = res.data.images[0].image_versions2.candidates[0].url;
    ...
    data = (await axios.get(`https://api.nguyenlienmanh.com/instagram/videodl?url=${link_video}`)).data,
    ...
    axios.get(`https://api.nguyenlienmanh.com/instagram/videodl?url=${link_post}`)/* request đến api */
