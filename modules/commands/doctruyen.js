module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Hakira",
    description: "Đọc Truyên Trên Messager Của Bạn ?",
...
module.exports.run = async function ({ api, event, args, Threads, Users }) {
    try {
        ...
        if (!args[0] || args[0].toUpperCase() == 'NEW') {
            const resp = await axios.get('https://truyennhanh1.com/')
            ...
            for (i = 0; i < 5; i++) {
                msg += `Tên: ${obj[i].name}\nLink: https://truyennhanh1.com${obj[i].link}\n\n`
        ...
        if (args[0] == 'find' || args[0] == 'sreach') {
            var names = args.splice(1).join(" ")
            const resp = await axios.get('https://truyennhanh1.com/tim-kiem/' + encodeURI(names))
            var $ = cheerio.load(resp.data)
    ...
    try {
      ...
      api.unsendMessage(handleReply.messageID);
        if (handleReply.step == 0) {
            const resp = await axios.get('https://truyennhanh1.com' + handleReply.info[event.body - 1].link)
            var $ = cheerio.load(resp.data)
            ...
            return api.sendMessage({
                ...
                return global.client.handleReply.push({
                    step: 1,
                    name: this.config.name,
                    link: 'https://truyennhanh1.com' + handleReply.info[event.body - 1].link,
                    leng: leng,
                    author: event.senderID,
                    messageID: info.messageID
                });
        ...
        if (handleReply.step == 1) {
            if (handleReply.leng < event.body) return api.sendMessage('Khong Duoc Chon Chapter Lon Hon Hien Co', event.threadID)
