...
module.exports.config = {
    ...
    usages: "[register|shop|bag|custom|help]",
    cooldowns: 0,
};

module.exports.checkPath = function (type, senderID) {
    const pathItem = path.join(__dirname, 'cache', 'mine', `item.json`);
    const pathUser = path.join(__dirname, 'cache', 'mine', 'datauser', `${senderID}.json`);
...
module.exports.run = async function ({
    ...
    const axios = require("axios")
    const pathData = path.join(__dirname, 'cache', 'mine', 'datauser', `${senderID}.json`);
    switch (args[0]) {
    case 'register':
    case '-r': {
    ...
    case 'help': {
            return api.sendMessage({body: "====[Minecraft]====\n- R: Đăng kí\n- CUSTOM: Lựa chọn khu vực đào\n- BAG: Xem túi đồ\n- SHOP: Cửa hàng\n\n__", attachment: await this.subnautica()}, threadID, messageID);
        }
    default: {
